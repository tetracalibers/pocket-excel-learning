...
import _data from "@/data/json/produce101japan-s1-original.json"
import data from "@/data/array/produce101japan-s1-original.json"
import ExcelDataSlice from "@/components/excel/ExcelDataSlice.astro"
import ExcelTableSlice from "@/components/excel/ExcelTableSlice.astro"
import { IF, SEARCH, ISERROR, TRIM, MID, LEN } from "@/lib/formulajs"

export const formula = (cvalue) =>
  IF(ISERROR(SEARCH("(????-??-??)", cvalue)), "非公開", TRIM(MID(cvalue, 13, LEN(cvalue))))
...
<ExcelDataSlice
  ...
  rows={[2, 101]}
  columns={["C"]}
  addColumns={[{ name: "D", header: "作業列", source: "C", formula }]}
/>
