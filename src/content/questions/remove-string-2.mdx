...
<ExcelDemoSlice
  source={{ data, sheetName: "sheet1" }}
  formula={{
    C2: '=IF(ISERROR(SEARCH("(????-??-??)", sheet1!C2)), "非公開", TRIM(MID(sheet1!C2, 13, LEN(sheet1!C2))))'
  }}
  pickRows={[1, 2, 3, 4, 101]}
  pickCols={["A", "C"]}
  emptyCols={["C"]}
...
Excel 2021以降やMicrosoft 365では、関数内で`C2:C102`のように複数のセルを一度に指定して、:kbd[Enter]を押すことで、C102セルまで一気に関数が実行される。（スピル機能）

:::details[解答例（スピル機能が使える場合）]

```elm caption="「C2」ではなく「C2:C102」のようにセル範囲を指定"
